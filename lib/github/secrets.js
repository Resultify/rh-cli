...
async function encryptSecrets (data) {
  ...
  try {
    const hubspotPortalIdSecretVal = data.prompts?.githubRepoFromTmpl?.repoSecrets?.hubspotPortalId || ''
    const hubspotPersonalAccessKeySecretVal = data.prompts?.githubRepoFromTmpl?.repoSecrets?.hubspotPersonalAccessKey || ''
